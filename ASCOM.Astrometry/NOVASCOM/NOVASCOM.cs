...
namespace ASCOM.Astrometry.NOVASCOM
{
   ...
   public class Earth : IEarth
   {
      ...
      private PositionVector m_HeliPos = new PositionVector();
      private VelocityVector m_BaryVel;
      private VelocityVector m_HeliVel = new VelocityVector();
      ...
      {
         double[] m_peb = new double[3];
         double[] m_veb = new double[3];
         double[] m_pes = new double[3];
         double[] m_ves = new double[3];
   ...
   public class Planet : IPlanet
   {
      ...
      private int[] m_ephdisps = new int[5];
      private int[] m_earthephdisps = new int[5];
      ...
      public PositionVector GetApparentPosition(double tjd)
      {
         ...
         double[] pos3 = new double[4];
         double[] pos4 = new double[4];
         double[] pos5 = new double[4];
         double[] vec = new double[9];
         int iter = 0;
         PositionVector pv = default(PositionVector);

         Object3 Obj3 = default(Object3);
         double RA = 0;
         double Dec = 0;
         double Dis = 0;
         int rc;
         ...
         if ((m_type == BodyType.MajorPlanet) && ((m_number == 10) || (m_number == 11))) // Handle Sun and Moon through NOVAS31,all the rest by the original Bob Denny method
         {
            ...
            rc = System.Convert.ToInt32(Nov31.AppPlanet(tjd, Obj3, Accuracy.Full, RA, Dec, Dis)); // Get the apparent RA/Dec
            ...
            do {
               ...
               t3 = tdb - lighttime;
               iter++;
            } while ((Abs(t3 - t2) > 0.000001) && iter < 100);
      ...
      public PositionVector GetAstrometricPosition(double tjd)
      {
         ...
         PositionVector RetVal = default(PositionVector);
         Object3 Obj3 = default(Object3);
         double RA = 0;
         double Dec = 0;
         double Dis = 0;
         ...
         if ((m_type == BodyType.MajorPlanet) && ((m_number == 10) || (m_number == 11))) // Handle Sun and Moon through NOVAS31,all the rest by the original Bob Denny method
         {
            ...
            rc = System.Convert.ToInt32(Nov31.AstroPlanet(tjd, Obj3, Accuracy.Full, RA, Dec, Dis)); // Get the astro RA/Dec
            Nov31.RaDec2Vector(RA, Dec, Dis, pos1); // Convert to a vector
      ...
      /// <remarks></remarks>
      public PositionVector GetLocalPosition(double tjd,
         Site site)
      {
         int j = 0;
         int iter = 0;
         SiteInfo st = default(SiteInfo);
         double t2 = 0;
         double t3 = 0;
         double gast = 0;
         double lighttime = 0;
         double ujd = 0;
         double[] pog = new double[4];
         double[] vog = new double[4];
         ...
         OnSurface OnSurf = new OnSurface();
         RefractionOption Ref3 = default(RefractionOption);
         short rc;
         double ra = 0;
         double rra = 0;
         double dec = 0;
         ...
         {
            m_deltat = System.Convert.ToDouble(DeltaTCalc(tjd));
         }
         ...
         if ((m_type == BodyType.MajorPlanet) && ((m_number == 10) || (m_number == 11))) // Handle Sun and Moon through NOVAS31,all the rest by the original Bob Denny method
         {
            ...
            rc = System.Convert.ToInt16(Nov31.LocalPlanet(tjd, Obj3, m_deltat, OnSurf, Accuracy.Full, ra, dec, dist));
      ...
      public PositionVector GetTopocentricPosition(double tjd, Site site, bool Refract)
      {
         short j = 0;
         int iter = 0;
         RefractionOption @ref = default(RefractionOption);
         SiteInfo st = default(SiteInfo);
         double ujd = 0;
         double t2 = 0;
         double t3 = 0;
         ...
         double[] pos4 = new double[4];
         double[] pos5 = new double[4];
         double[] pos6 = new double[4];
         double[] vel1 = new double[4];
         double[] vel2 = new double[4];
         double[] pog = new double[4];
         double[] vog = new double[4];
         double[] pob = new double[4];
         double[] vec = new double[4];
         double[] vob = new double[4];
         ...
         if ((m_type == BodyType.MajorPlanet) && ((m_number == 10) || (m_number == 11))) // Handle Sun and Moon through NOVAS31,all the rest by the original Bob Denny method
         {
            ...
            rc = System.Convert.ToInt16(Nov31.TopoPlanet(tjd, Obj3, m_deltat, OnSurf, Accuracy.Full, ra, dec, dist));
            Nov31.Equ2Hor(ujd, m_deltat, Accuracy.Full, 0.0, 0.0, OnSurf, ra, dec, Ref3, zd, az, rra, rdec);
         ...
         else // Some other planet
         {
            ...
            dist = System.Convert.ToDouble(Sqrt(Pow(vec[0], 2.0) + Pow(vec[1], 2.0) + Pow(vec[2], 2.0))); //And dist
            ...
            if (@ref != RefractionOption.NoRefraction) {
               RADec2Vector(rra, rdec, dist, vec); //If refracted, recompute New refracted vector
            }
      ...
      public PositionVector GetVirtualPosition(double tjd)
      {
         ...
         if ((m_type == BodyType.MajorPlanet) && ((m_number == 10) || (m_number == 11))) // Handle Sun and Moon through NOVAS31,all the rest by the original Bob Denny method
         {
            ...
            rc = System.Convert.ToInt32(Nov31.VirtualPlanet(tjd, Obj3, Accuracy.Full, RA, Dec, Dis)); // Get the astro RA/Dec
            Nov31.RaDec2Vector(RA, Dec, Dis, pos1); // Convert to a vector
            ...
            BodyType km_type = default(BodyType);
            if (m_type == BodyType.Comet) {
               km_type = BodyType.Comet;
            }
            else if (m_type == BodyType.MajorPlanet) {
      ...
      public int Number
      {
         ...
         set
         {
            ...
            if ((m_type == BodyType.MajorPlanet) && ((value < 1) || (value > 11))) {
               throw (new Utilities.Exceptions.InvalidValueException("Planet.Number MajorPlanet number is < 1 or > 11 - " + System.Convert.ToString(value)));
            }
            m_number = value;
         }
      }
      ...
      public BodyType Type
      {
         ...
         {
            if ((value < 0) || (value > 2)) {
               throw (new Utilities.Exceptions.InvalidValueException("Planet.Type BodyType is < 0 or > 2: " + System.Convert.ToString(value)));
            }
            m_type = value;
         }
      }
   ...
   /// </para></remarks>
   [Guid("8D8B7043-49AA-40be-881F-0EC5D8E2213D"), ClassInterface(ClassInterfaceType.None), ComVisible(true)]
   [Obsolete("This class will be withdrawn in the next major release, please use the SOFA or NOVAS31 classes instead")]
   public class PositionVector : IPositionVector, IPositionVectorExtra
   {
      ...
      public void Aberration(VelocityVector vel)
      {
         ...
         try {
            v[2] = System.Convert.ToDouble(vel.z);
         }
         catch (Exception) {
      ...
      {
         const double f = 0.00335281; //		// f = Earth ellipsoid flattening
         double df2 = 0;
         double t = 0;
         double sinphi = 0;
         double cosphi = 0;
         ...
         ////
         df2 = System.Convert.ToDouble(Pow((1.0 - f), 2));
         try {
            t = site.Latitude;
         }
         catch (Exception) {
         ...
         t = System.Convert.ToDouble(DEG2RAD * t);
         sinphi = System.Convert.ToDouble(Sin(t));
         cosphi = System.Convert.ToDouble(Cos(t));
         c = System.Convert.ToDouble(1.0 / Sqrt(Pow(cosphi, 2.0) + df2 * Pow(sinphi, 2.0)));
         s = df2 * c;
         ...
         ach = System.Convert.ToDouble(System.Convert.ToDouble(EARTHRAD * c) + t);
         ash = System.Convert.ToDouble(System.Convert.ToDouble(EARTHRAD * s) + t);
         ...
         stlocl = System.Convert.ToDouble((gast * 15.0 + t) * DEG2RAD);
         sinst = System.Convert.ToDouble(Sin(stlocl));
         cosst = System.Convert.ToDouble(Cos(stlocl));

         ////
         //// Compute position vector components in AU
         ////

         PosVec[0] = System.Convert.ToDouble((ach * cosphi * cosst) / KMAU);
         PosVec[1] = System.Convert.ToDouble((ach * cosphi * sinst) / KMAU);
         PosVec[2] = System.Convert.ToDouble((ash * sinphi) / KMAU);
      ...
      public bool SetFromStar(Star star)
      {
         ...
         ////
         try {
            paralx = System.Convert.ToDouble(star.Parallax);
         }
         catch (Exception) {
         ...
         ////
         m_Dist = System.Convert.ToDouble(RAD2SEC / paralx);
         try {
            m_RA = System.Convert.ToDouble(star.RightAscension);
         }
         catch (Exception) {
         ...
         try {
            m_DEC = System.Convert.ToDouble(star.Declination);
         }
         catch (Exception) {
         ...
         cra = System.Convert.ToDouble(Cos(r));
         sra = System.Convert.ToDouble(Sin(r));
         cdc = System.Convert.ToDouble(Cos(d));
         sdc = System.Convert.ToDouble(Sin(d));
      ...
      private void CheckEq()
      {
         ...
         m_Dist = System.Convert.ToDouble(Sqrt(Pow(PosVec[0], 2) + Pow(PosVec[1], 2) + Pow(PosVec[2], 2)));
         m_Light = m_Dist / C;
         RADecOk = true;
      }
      #endregion
   ...
   [Guid("46ACFBCE-4EEE-496d-A4B6-7A5FDDD8F969"), ClassInterface(ClassInterfaceType.None), ComVisible(true)]
   [Obsolete("This class will be withdrawn in the next major release, please use the SOFA or NOVAS31 classes instead")]
   ...
   public class Star : IStar
   {
      ...
      private short hr;
      double[] m_earthephdisps = new double[5];
      ...
      public PositionVector GetApparentPosition(double tjd)
      {
         ...
         hr = System.Convert.ToInt16(GetEarth(tjd, m_earth, tdb, peb, veb, pes, ves));
         if (hr > 0) {
            vec[0] = 0.0;
            vec[1] = 0.0;
            vec[2] = 0.0;
      ...
      public PositionVector GetTopocentricPosition(double tjd, Site site, bool Refract)
      {
         ...
         //// Distance does not change...
         ////
         if (@ref > 0) //If refracted, recompute
         {
            RADec2Vector(rra, rdec, dist, vec); //New refracted vector
         }

         //Create a new positionvector with calculated values
         PositionVector PV = new PositionVector(vec[0], vec[1], vec[2], rra, rdec, dist, dist / C, az, (90.0 - zd));
      ...
      public void SetHipparcos(double RA, double Dec, double ProMoRA, double ProMoDec, double Parallax, double RadVel)
      {
         ...
         m_ra = System.Convert.ToDouble(fk5.RA);
         m_dec = System.Convert.ToDouble(fk5.Dec);
         m_pmra = System.Convert.ToDouble(fk5.ProMoRA);
         m_pmdec = System.Convert.ToDouble(fk5.ProMoDec);
         m_plx = System.Convert.ToDouble(fk5.Parallax);
         m_rv = System.Convert.ToDouble(fk5.RadialVelocity);
         m_rav = true;
         m_decv = true;
         m_num = 0;
         m_name = ""; //\0';
   ...
   [Guid("25F2ED0A-D0C1-403d-86B9-5F7CEBE97D87"), ClassInterface(ClassInterfaceType.None), ComVisible(true)]
   [Obsolete("This class will be withdrawn in the next major release, please use the SOFA or NOVAS31 classes instead")]
   public class VelocityVector : IVelocityVector, IVelocityVectorExtra
   {
      ...
      {
         const double f = 0.00335281; //f = Earth ellipsoid flattening
         const double omega = 0.000072921151467; //omega = Earth angular velocity rad/sec
         double df2 = 0;
         double t = 0;
         double sinphi = 0;
         double cosphi = 0;
         ...
         df2 = System.Convert.ToDouble(Pow((1.0 - f), 2));
         try {
            t = System.Convert.ToDouble(site.Latitude);
         }
         catch (Exception) {
            throw (new Exceptions.ValueNotAvailableException("VelocityVector:SetFromSite Site.Latitude is not available"));
         }

         t *= System.Convert.ToDouble(DEG2RAD);
         sinphi = System.Convert.ToDouble(Sin(t));
         cosphi = System.Convert.ToDouble(Cos(t));
         c = System.Convert.ToDouble(1.0 / Sqrt(Pow(cosphi, 2.0) + df2 * Pow(sinphi, 2.0)));
         s = df2 * c;
         try {
            t = System.Convert.ToDouble(site.Height);
         }
         catch (Exception) {
         ...
         ////
         try {
            t = System.Convert.ToDouble(site.Longitude);
         }
         catch (Exception) {
         ...
         m_v[0] = System.Convert.ToDouble((-omega
            * ach * cosphi * sinst) * 86400.0 / KMAU);
         m_v[1] = System.Convert.ToDouble((omega * ach * cosphi * cosst) * 86400.0 / KMAU);
         m_v[2] = 0.0;
      ...
      public bool SetFromStar(Star star)
      {
         ...
         d *= System.Convert.ToDouble(DEG2RAD);
         ...
         ////
         m_v[0] = System.Convert.ToDouble(-m_VRA * sra - m_VDec * sdc * cra + m_RadVel * cdc * cra);
      ...
      private void CheckEq()
      {
         ...
         m_RadVel = System.Convert.ToDouble(Sqrt(Pow(m_v[0], 2) + Pow(m_v[1], 2) + Pow(m_v[2], 2)));
         m_cv = true;
      }
      #endregion
