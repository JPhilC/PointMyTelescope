...
namespace Lunatic.Core.Geometry
{
   ...
   /// </summary>
   public class Angle
      : DataObjectBase, IComparable, IEquatable<Angle>
   {
      ...
      */
      ...
      public double Radians
      {
         ...
         set
         {
            if (Set<double>("Radians", ref _Value, Angle.RadiansToDegrees(value))) {
               _HasBeenSet = true;
               SetDmsFromDegrees(Value);
      ...
      public int Degrees
      {
         ...
         set
         {
            if (Set<int>("Degrees", ref _Degrees, value)) {
               MatchDmsSigns(value);
               _Value = DmsToDegrees(Degrees, Minutes, Seconds);
               _HasBeenSet = true;
               RaisePropertyChanged("Value");
               RaisePropertyChanged("Radians");
            }
   ...
   [DefaultValue(0.0)]
      ...
      public static bool operator ==(Angle angle1, Angle angle2)
      {
         ...
         if (ReferenceEquals(angle1, null)) {
            return false;
         }
         if (ReferenceEquals(angle2, null)) {
            return false;
         }
      ...
      public static Angle operator +(Angle angle1, Angle angle2)
      {
         ...
         if (angle1.Format == AngularFormat.DecimalDegrees
             || angle1.Format == AngularFormat.DecimalDegreesWestEast) {
            newAngle = new Angle(angle1.Value + angle2.Value);    /* Use Value property to ensure DMS properties are also updated properly */
         }
         else {
            double seconds = angle1.TotalSeconds + angle2.TotalSeconds;
            newAngle = FromSeconds(seconds);
         }
      ...
      public static Angle operator *(Angle angle, double factor)
      {
         ...
         if (angle.Format == AngularFormat.DecimalDegrees
            ...
            newAngle = new Angle(angle.Value * factor);  /* Use Value property to ensure DMS properties are also updated properly */
         }
         else {
      ...
      private void SetDmsFromDegrees(double angle)
      {
         ...
         double minutes = (angle - _Degrees) * 60.0;
         _Minutes = (int)Truncate(minutes);
      ...
      private double GetDegreesFromDms()
      {
         ...
         return (double)_Degrees + _Minutes / 60.0 + (_Seconds / 3600.0);
      }
